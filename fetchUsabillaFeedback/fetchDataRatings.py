...
def sheets(service, values, SPREADSHEET_ID, rangeName):
...
def export():
    ...
    numbermap = {'createdAt':1, 'createdBy':2, 'id':3, 'rating':4, 'feedback':5, 'userId':6, 'videoId':7 } 
    final_dict = {}
    values = []
    for feedback_dict in feedbacks_list:
        flat_dict = flatten_json(feedback_dict)
        if 'feedback' not in flat_dict:
            flat_dict['feedback'] = ''
        for item in flat_dict:
            if item == 'createdAt':
                flat_dict[item]=flat_dict[item][1:10]
...
if __name__ == '__main__':
    ...
    values = export()
    sheets(service, values, spreadsheetIds['Usabilla GS'], 'Ratings')
